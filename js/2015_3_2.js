...
function followDirection(directions) {
  ...
  for (let index = 0; index < santas.length; index++)
    houses.add(generateKey(santas[index]))
  ...
  while (cursor < directions.length) {
    const nextPosition = nextHouse(santas[currentSanta], directions[cursor])
    santas[currentSanta] = nextPosition
    houses.add(generateKey(nextPosition))
    cursor++
    currentSanta = Number(!currentSanta)
  }
